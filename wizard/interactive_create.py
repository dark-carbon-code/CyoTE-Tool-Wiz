...
from utils.file_utils import save_tool_json, save_chunks_for_tool
...
from utils.deployment_context import select_trl_level, multi_select_context
from rich import print
from utils.chunker import extract_chunks_for_rag
from utils.rag_formatter import generate_markdown_summary
from utils.md_chunk_exporter import export_tool_to_md_chunks
from pathlib import Path
import os
import uuid

DB_PATH = "C:/Projects/cyote_tool_wiz/data/tool_knowledge.db"
...
def create_tool():
    ...
    tool_name = inquirer.text(message="\U0001F6E0 Tool name:").execute()
    description = inquirer.text(message="\U0001F4D8 Describe what this tool does and how it's used:").execute()

    selected_roles = select_tool_roles()

    print(f"\n[bold yellow]\U0001F4CB Preview of selected user roles with NICE alignment:[/bold yellow]")
    ...
    if autogenerated_use_cases:
        print(f"\n[bold magenta]\U0001F52C Auto-generated {len(autogenerated_use_cases)} CSF use cases based on NICE roles:[/bold magenta]")
        for uc in autogenerated_use_cases:
            print(f" - [cyan]{uc['csf_function_id']}[/cyan] [bold]{uc['name']}[/bold]")

    matrix_choice = inquirer.select(
        message="\U0001F9FD Choose MITRE matrix:",
    ...
    based_on_pars = inquirer.confirm(
        message="\U0001F4CA Is this tool based on the CyOTE Precursor Analysis Reports (PARs)?",
        default=False
    ).execute()
    ...
    if based_on_pars:
        ...
        selected_tactic_ids = inquirer.checkbox(
            message="\U0001F3AF Select supported tactics:",
            choices=[f"{t['id']} - {t['name']}" for t in tactics]
        ).execute()

        known_input = inquirer.text(
            message="\U0001F50D Known Technique IDs or Names (comma-separated):", default=""
        ).execute()
            ...
            for tactic in selected_tactic_ids:
                ...
                if techs:
                    manual_selection = inquirer.checkbox(
                        message=f"\U0001F4CC Techniques under [bold]{tactic}[/bold]:",
    ...
    trl_info["commercial_viable"] = bool(trl_info.get("commercial_viable"))
    trl_info["commercial_viable_label"] = "Yes" if trl_info["commercial_viable"] else "No"

    host_env_info = multi_select_context("Hosting Environments", "hosting_env")
    ...
    example_usage = inquirer.text(message="\U0001F4CC Paste example usage from doc or README:").execute()
    github_link = inquirer.text(message="\U0001F517 GitHub URL (if available):").execute()
    fact_sheet = inquirer.text(message="\U0001F4C4 Fact Sheet or PDF URL (if available):").execute()
    ...
    json_path = save_tool_json(tool_entry)
    save_chunks_for_tool(tool_entry, json_path)

    markdown = generate_markdown_summary(tool_entry)
    markdown_dir = os.path.join("data", "tool_markdown_summaries")
    os.makedirs(markdown_dir, exist_ok=True)
    markdown_path = os.path.join(markdown_dir, os.path.basename(json_path).replace(".json", ".md"))
    with open(markdown_path, "w", encoding="utf-8") as f:
        f.write(markdown)

    export_tool_to_md_chunks(tool_entry)

    print(f"\n[bold blue]✔ Tool entry for '{tool_name}' saved to JSON, .md, and sectioned Markdown![/bold blue]")
