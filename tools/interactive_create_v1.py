...
def create_tool():
    ...
    tactics = query_db("""
        ...
        JOIN matrices m ON t.matrix_id = m.id
        WHERE m.name = ?
    """, (matrix_choice,))
    ...
    ds_results = query_db("""
        ...
        JOIN matrices m ON ds.matrix_id = m.id
        WHERE m.name = ?
    """, (matrix_choice,))
    selected_ds = inquirer.checkbox(
    ...
    # Use Cases (save names not just IDs)
    ...
    selected_use_cases = inquirer.checkbox(
        ...
        choices=[{"name": uc["name"], "value": uc["name"]} for uc in use_cases]
    ).execute()

    # Deployment Context
    dep_context = query_db("SELECT id, label, description, category FROM tool_deployment_context")
    selected_dep_contexts = inquirer.checkbox(
        message="🚀 Select deployment context details (inputs/outputs/hosting/interface):",
        choices=[{
            "name": f"{row['category']} - {row['label']} → {row['description']}",
            "value": {
                "category": row["category"],
                "label": row["label"],
                "description": row["description"]
            }
        } for row in dep_context],
        instruction="(Choose all that apply for how the tool operates and integrates)"
    ).execute()

    # Free Text Fields
    example_usage = inquirer.text(
        message="📌 Paste example usage, scenario walkthrough, or doc excerpt:"
    ).execute()

    github_link = inquirer.text(
        message="🔗 Paste GitHub repo URL or leave blank:"
    ).execute()

    fact_sheet = inquirer.text(
        message="📄 Paste fact sheet or documentation link or leave blank:"
    ).execute()
    ...
    tool_entry = {
        ...
        "data_sources": selected_ds,
        "observable_types": selected_obs_types,
        "use_cases": selected_use_cases,
        "deployment_context": selected_dep_contexts,
        "example_usage": example_usage.strip(),
        "github": github_link.strip() or None,
        "factsheet": fact_sheet.strip() or None
    }
